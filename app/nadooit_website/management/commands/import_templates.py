...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        input_dir = os.path.join(settings.BASE_DIR, 'nadooit_website', 'sections_templates')
        parser.add_argument('--input-dir', type=str, default=input_dir, help='Input directory for the templates to import')
            ...
            for filename in files:
                if filename.endswith('.html'):
                    if not self.is_valid_filename(filename):
                        ...
                        self.stdout.write(self.style.SUCCESS(f'Renamed file "{old_path}" to "{new_path}".'))
                    ...
                    self.stdout.write(self.style.SUCCESS(f'Successfully imported template "{section_name}"'))